...
<content>
            ...
            <v-row justify="center" class="ma-0 pa-0">
                <v-col cols="10" class="ma-0 pa-0">
                    ...
                    <v-form ref="form" v-model="valid">
                        <!-- mail入力 -->
                        <v-text-field v-model="mailaddress" class="ma-0 pa-0 pb-6" prepend-icon="mdi-email" :rules="registemailRules" label="E-mail" hint="メールアドレスは50字以下で記入してください。" counter required />
                        <!-- パスワード入力 -->
                        <v-text-field v-model="password" class="ma-0 pa-0 pb-6" prepend-icon="mdi-lock" :rules="registpassRules" :type="showPassword ? 'text' : 'password'" v-bind:append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'" @click:append="showPassword = !showPassword" label="password" hint="パスワードは8字以上20字以下にしてください。" counter required />
                        <!-- ログインボタン -->
                        <v-row justify="center" align="center" class="ma-0 pa-0 pb-2">
                            <v-btn class="ma-0 pa-0 white--text" style="background-color: #83B590" @click="login" width="120" dark outlined rounded :disabled="!valid" :loading="loading">
                                ログイン
                            </v-btn>
    ...
    <!-- 新規登録ページボタン -->
    <v-btn dark elevation="0" style="background-color: #A5D6A7; color: #FFF" to="/regist">
        新規登録
    </v-btn>
</content>
</template>
